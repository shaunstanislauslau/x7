...
public class JedisConnector_Cache3 {
	...
	public static JedisConnector_Cache3 getInstance(){
		if (instance == null){
			...
			String password= Configs.getString("x7.redis.password.cache3");
			
			if (StringUtil.isNullOrEmpty(password)) {
				pool = new JedisPool( config, Configs.getString("x7.redis.ip.cache3"), Configs.getIntValue("x7.redis.port.cache3"));  //6379
				
				JedisTest.test(pool, "Redis start fail without password, instance of: " + JedisConnector_Cache3.class.getName());
			}else {
				pool = new JedisPool( config, Configs.getString("x7.redis.ip.cache3"), Configs.getIntValue("x7.redis.port.cache3"),
					1000,password);  //6379
				
				JedisTest.test(pool, "Redis start fail with password: "+password + ", instance of: " + JedisConnector_Cache3.class.getName());
